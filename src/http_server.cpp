...
<section class="grid"><div class="card"><h2>Function latency</h2><table><thead><tr><th>Function</th><th>Samples</th><th>Mean</th><th>p99</th><th>Cache / event</th></tr></thead><tbody id="metrics"></tbody></table></div><div class="card"><h2>3σ alerts</h2><div id="alerts"><p class="empty">Establishing baseline…</p></div></div></section>
...
std::string metrics_json(const CorrelationEngine& engine) {
  ...
  for (const auto& metric : engine.metrics()) {
    ...
    json += "{\"function\":\"" + metric.function + "\",\"samples\":" + std::to_string(metric.samples) +
        ...
        json_number(metric.p99_latency_ns) + ",\"cache_misses_per_event\":" + json_number(metric.cache_misses_per_event) +
